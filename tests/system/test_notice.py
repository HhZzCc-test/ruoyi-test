...
@allure.story("通知公告接口")
class TestNoticeManagement(BaseTest):

    @pytest.fixture(autouse=True)
    def _setup(self, auth_token):
        self._token = auth_token

    @allure.title("通知列表 - 正常场景: 获取通知列表")
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_get_notice_list(self):
        resp = self.client.get_notice_list()
    ...
    @allure.title("通知列表 - 权限场景: 无Token访问")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_get_notice_list_no_auth(self):
        resp = self.client.session.get(
            self.client._url("/system/notice/list"),
            headers={"Content-Type": "application/json"}
        )
        self.assert_http_ok(resp)
        self.assert_business_success(resp.json())

    @allure.title("通知列表 - 权限场景: 无效Token访问")
    @allure.severity(allure.severity_level.NORMAL)
    def test_get_notice_list_invalid_token(self):
        resp = self.client.session.get(
            self.client._url("/system/notice/list"),
